...
class ImportMemcachedServer(ImportEc2Server):
	...
	def _install_software(self, channel, distr):
		...
		prefix, software  = ('apt-get -y install ',['memcached']) if debian else ('yum -y install ', ['memcached'])
		...
		if debian:
			...
			if not re.search('Complete!|Nothing to do', out):
				raise Exception('Cannot install memcached')
			
		ImportEc2Server._install_software(self, channel, distr)
			
	def _get_role_name(self):
		return 'Test-memcd-%s' % time.strftime('%Y-%m-%d-%H%M')
	
	def _import_server(self, role_name):
		return import_server(get_selenium(), ScalrConsts.Platforms.PLATFORM_EC2 ,\
			ScalrConsts.Behaviours.BEHAVIOUR_MEMCACHED, self.ip_address, role_name)	
	#temporary!		
	def _change_behaviour(self, import_server_str):
		return re.sub('behaviour=[\w-]*', 'behaviour=memcached', import_server_str)
...
class TestImportAppServer(unittest.TestCase):
	...
	def setUp(self):
		self.importer = ImportMemcachedServer(sysargs)
