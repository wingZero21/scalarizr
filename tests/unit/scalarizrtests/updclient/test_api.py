...
LOCK_FILE = SANDBOX_DIR + '/private.d/update.lock'
...
class TestUpdClientAPI(object):
	...
	def setup(self):
		...
		upd_api.UpdClientAPI.lock_file = LOCK_FILE
	...
	def setup_sync(self, queryenv=None, metadata=None, **kwds):
		...
		upd = self.setup_bootstrap(metadata, queryenv, lock_file_data=self.LOCK_FILE_1, **kwds)
		upd.bootstrap()
		return upd
